...
def get_context():
    ...
    return "夜間（前頭前野のブレーキが弱まり、扁桃体が暴走しやすい時間）" if is_night else "日中"
...
# --- STEP 2: 思考ログの全出力 ---
    ...
    with col_in1:
        st.markdown("### 🟢 本音（素直）")
        st.session_state.sunao_input = st.text_area("「本当はどうしたい？」", placeholder="例：もう休みたい、一人の時間が欲しい...", height=200)
    with col_in2:
        st.markdown("### 🔴 義務（社会性）")
        st.session_state.social_input = st.text_area("「〜しなきゃ、〜すべき」", placeholder="例：期待に応えなきゃ、失礼のないようにしなきゃ...", height=200)
    
    if st.button("2.5 Flash 調律プロセスを実行 ➔", type="primary"):
        with st.spinner("ハードウェアとソフトウェアを照合中..."):
            try:
                ...
                【2.5 Flash 調律ガイドライン】
                1. 「素直」と「社会性」の脳内シェア（100%）を推定。
                2. 睡眠、食事、スマホ利用、安全基地の有無が「義務感」や「しんどさ」をどう増幅させているか物理的に解説。
                3. 生活習慣を改善し、OSを安定させるための具体的な『処方箋』を3つ提案してください。

                【JSON構造】
                {{
                    "sunao_pct": 0-100, "social_pct": 0-100, "sunao_tag": "本音の短縮タグ", "social_tag": "義務の短縮タグ",
