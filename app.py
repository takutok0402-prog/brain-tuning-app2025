...
# --- STEP 2: 脳内ログ & 今日の光 ---
    ...
    with col_in1:
        ...
        st.session_state.sunao_input = st.text_area("今の本音・理想（空欄OK）", height=200, key="sunao_t", placeholder="例：英語しゃべれるようになりたい。勉強に集中するんだ...")
    with col_in2:
...
# --- STEP 3: レポート ---
    ...
    ax.axis('off')
    st.pyplot(fig)
    st.caption(f"🔵 自分軸: {r}% / 🟠 外部軸: {100-r}% (AI Judge)")
