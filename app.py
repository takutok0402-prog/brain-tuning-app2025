...
if st.session_state.step == 1:
    ...
    st.markdown("小池さん、今のマシンのコンディションを教えてください。")
    
    st.subheader("🔋 バイタル・リソース")
    v_col1, v_col2 = st.columns(2)
    with v_col1:
        st.session_state.fatigue_val = st.select_slider("😫 疲労度・眠気", options=["絶好調", "普通", "少し消耗", "シャットダウン寸前"], value="普通")
    with v_col2:
        st.session_state.hunger_val = st.select_slider("🍕 エネルギー（空腹）", options=["満タン", "普通", "低下", "ガス欠"], value="普通")
    
    st.divider()
    st.markdown("##### 🧠 脳内マップの座標")
    ...
    st.session_state.social_filter_val = st.radio("誰かの視線や『普通は〜』という期待をどのくらい感じていますか？", 
                             ["0%（素直に全振り）", "50%（調整中）", "100%（社会性が脳を占拠）"], index=1)

    if st.session_state.selected_emotion != "(選択してください)":
        if st.button("2.5 Flash でデバッグを開始 ➔", type="primary"):
            move_to(2)
    ...
    if st.button("2.5 Flash 調律プロセスを実行 ➔", type="primary"):
        with st.spinner("思考の境界線をスキャンしています..."):
            try:
                ...
                prompt = f"""
                【解析対象】
                - 素直な想い: {st.session_state.sunao_input}
                - 社会的な義務: {st.session_state.social_input}
                - コンディション: {get_context()}, 疲労={st.session_state.fatigue_val}, 空腹={st.session_state.hunger_val}

                【2.5 Flash 調律ガイドライン】
                1. ユーザーの入力を元に「素直」と「社会性」の脳内シェア（合計100%）を推定してください。
                2. 疲労や夜間といった「一時的なデバッグ要因」が、どのように『義務感』を過剰に重く見せているか論理的に解説してください。
                3. 決めつけを排し、ユーザーが今抱えている葛藤を、彼自身の『誠実さ』の証として深く肯定してください。
                4. 自律を取り戻すための具体的な「損切り（リセット）」案を提示してください。
                ...
                {{
                    ...
                    "hardware_effect": "身体要因が義務感に与えている影響",
                    "validation": "誠実さのバリデーション",
...
# --- STEP 3: 調律結果の可視化 ---
    ...
    # テキスト配置
    ax.text(0.3, 0.5, f"本音(素直)\n{s_pct}%\n『{scan['sunao_tag']}』", ha='center', va='center', fontweight='bold', fontsize=10)
    ax.text(0.7, 0.5, f"義務(社会性)\n{so_pct}%\n『{scan['social_tag']}』", ha='center', va='center', fontweight='bold', fontsize=10)
